...
fn greedyMeshWithFastExits(alloc: std.mem.Allocator, state: *MeshingState, world: *const World, pos: Chunk.Pos) void {
    ...
    defer _ = mesh_time_ns.fetchAdd(timer.read(), .monotonic);
...
fn greedyMesh(alloc: std.mem.Allocator, state: *MeshingState, refs: ChunkRefs) void {
    ...
    _ = get_opaque_ns.fetchAdd(timer.read(), .monotonic);
    timer = std.time.Timer.start() catch @panic("");
    ...
    _ = edge_detect_ns.fetchAdd(timer.read(), .monotonic);
    timer = std.time.Timer.start() catch @panic("");
    defer _ = mesh_remaining_time_ns.fetchAdd(timer.read(), .monotonic);
