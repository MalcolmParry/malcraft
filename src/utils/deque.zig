//! copied from zig master
//! will delete when added to next release of zig
...
pub fn Deque(comptime T: type) type {
    return struct {
        ...
        pub fn ensureTotalCapacity(deque: *Self, gpa: Allocator, new_capacity: usize) Allocator.Error!void {
            ...
            // src/deque.zig:62:74: error: 'growCapacity' is not marked 'pub'
            //             return deque.ensureTotalCapacityPrecise(gpa, std.ArrayList(T).growCapacity(new_capacity));

            // return deque.ensureTotalCapacityPrecise(gpa, std.ArrayList(T).growCapacity(new_capacity));
            return deque.ensureTotalCapacityPrecise(gpa, @max(new_capacity, deque.buffer.len));
        }
